...
wholesale_el_prices <- read_csv("R:/SFBWISO/RE Deployment/3. To Submit (AIP)/02. Data & Code/0.Data/wholesale_electricity_prices.csv") %>%
  ...
  # Pivot longer to wider format
  pivot_longer(cols = starts_with("19") | starts_with("20"), names_to = "year", values_to = "prices") %>%
  # Convert the 'year' column to numeric and handle missing values
  mutate(year = as.numeric(year),
...
#convert country names to codes
wholesale_el_prices$country<-countrycode(wholesale_el_prices$country, "country.name", "iso3c") 
#rearrange columns
wholesale_el_prices<-wholesale_el_prices%>% 
  relocate(country, .before = year) %>% 
  arrange(country, year) 

pwholesale_el_prices<- pdata.frame(wholesale_el_prices, index = c("country", "year"))

#plot detection
ggplot(pwholesale_el_prices, aes(x=year, y=prices, group=country, color=country)) +
  geom_line() +
  facet_wrap(~country) +
  labs(title = "Wholesale electricity prices by country", 
       x = "Year", y = "EUR/kWh") +
  theme_minimal()

ggplot(pwholesale_el_prices, aes(x=year, y=log(prices), group=country, color=country)) +
  geom_line() +
  facet_wrap(~country) +
  labs(title = "Wholesale electricity prices by country", 
       x = "Year", y = "log(prices)") +
...
data_test[, lprices:=log(prices), by="country"]


#### Output
write.csv(data_test, "WinddepDrivers_dataset_robustness_checks_w_prices.csv", row.names = F)
...
rm(wind_capacity,pwind_capacity,year, wind_costs, wind_costs_test, ltir, pltir,
   ...
   uk_wind_final, wind_capacity_smooth, gdp_pc, pgdp_pc, power_demand, 
   ppower_demand, ff_share, pff_share, wholesale_el_prices, pwholesale_el_prices)
...
ltitle1="Results_WindDrivers_Analysis_robustness_checks_w_prices.txt"

# Prepare Data

data_test <- read.csv("WinddepDrivers_dataset_robustness_checks_w_prices.csv")
data_test <- as.data.table(data_test)
...
correlation_matrix<-c(cor(data_test$lwind.additions, data_test$lltir, use="complete.obs"),
                      ...
                      cor(data_test$lwind.additions, data_test$lgdp.pc, use="complete.obs"),
                      cor(data_test$lwind.additions, data_test$lpower.demand, use="complete.obs"),
                      cor(data_test$lwind.additions, data_test$fossil_fuel_share, use="complete.obs"),
                      cor(data_test$lwind.additions, data_test$lprices, use="complete.obs"))
...
for(p.value in c(.05, .02, .01)){
  ...
  is1 <- isatpanel(
    data = dat_test,
    formula = "lwind.additions ~ lltir + lgdp.pc + lwind.costs + lpower.demand + fossil_fuel_share + lprices", 
...
robust_is1<-robust_isatpanel(is1, HAC = T)
print(robust_is1)

#prices are insignificant when using all variables;
#next:specification with prices and the variables in the baseline model only

# analysis

set.seed(1230) 

ltitle2="Results_WindDrivers_Analysis_robustness_checks_w_prices_baseline_vrbls.txt"
...
for(p.value in c(.05, .02, .01)){
  ...
  is2 <- isatpanel(
    data = dat_test,
    formula = "lwind.additions ~ lltir + lwind.costs + lprices", 
    index = c("country", "year"),
    effect = "individual",
    iis = T,
    tis = T,
...
#robust standard errors 
robust_is2<-robust_isatpanel(is2, HAC = T)
print(robust_is2)