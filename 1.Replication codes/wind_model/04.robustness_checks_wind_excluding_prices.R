...
ggplot(wind_costs_test, aes(x=year, y=log(onshore_wind_costs))) +
  geom_line() +
  labs(title = "Global onshore wind costs", 
       x = "Year", y = "log($/KW)") +
  theme_minimal()
...
#### Output
write.csv(data_test, "WinddepDrivers_dataset_robustness_checks_excl_prices.csv", row.names = F)

### clean working directory
rm(ltir, pltir, wind_capacity,pwind_capacity,wind_costs, pgdp_pc,gdp_pc, wind_costs_test, 
   uk_wind, uk_euro_wind, uk_eurostat_wind, uk_wind_final, uk_spliced_wind,
   wind_capacity_smooth, irena_data, power_demand, ppower_demand, ff_share, pff_share)
...
ltitle1="Results_WindDrivers_Analysis_robustness_checks_excl_prices.txt"

# Prepare Data

data_test <- read.csv("WinddepDrivers_dataset_robustness_checks_excl_prices.csv")
data_test <- as.data.table(data_test)
...
for(p.value in c(.05, .02, .01)){
  ...
  is <- isatpanel(
    data = dat_test,
    formula = "lwind.additions ~ lltir + lgdp.pc + lwind.costs + lpower.demand + fossil_fuel_share", 
