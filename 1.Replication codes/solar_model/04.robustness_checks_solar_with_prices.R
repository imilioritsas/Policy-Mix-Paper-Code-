...
#### Output
write.csv(data_test, "SolardepDrivers_dataset_robustness_checks_w_prices.csv", row.names = F)

### clean working directory
rm(ltir, pltir, solar_capacity,psolar_capacity,solar_capex_all, small_non_residential_solar_CAPEX,
   ...
   solar_capacity_smooth, irena_data, power_demand, ppower_demand, ff_share, pff_share, wholesale_el_prices, pwholesale_el_prices)
...
ltitle1="Results_SolarDrivers_Analysis_robustness_checks_with_prices.txt"

# Prepare Data

data_test <- read.csv("SolardepDrivers_dataset_robustness_checks_w_prices.csv")
data_test <- as.data.table(data_test)
...
correlation_matrix<-c(cor(data_test$lsolar.additions, data_test$lltir, use="complete.obs"),
                      ...
                      cor(data_test$lsolar.additions, data_test$lprices, use="complete.obs"))
...
for(p.value in c(.05, .02, .01)){
  ...
  is1 <- isatpanel(
    data = dat_test,
    formula = "lsolar.additions ~ lltir + lgdp.pc + lsolar.costs.adj + lpower.demand + fossil_fuel_share + lprices", 
...
ltitle2="Results_SolarDrivers_Analysis_robustness_checks_w_prices_baseline_vrbls.txt"
...
for(p.value in c(.05, .02, .01)){
  ...
  is2 <- isatpanel(
    data = dat_test,
    formula = "lsolar.additions ~ lltir + lgdp.pc + lsolar.costs.adj + lprices", 
    index = c("country", "year"),
    effect = "individual",
    iis = T,
    tis = T,
